 ...
 * 選報名方式那一步要先講清楚的事。
 *
 * 系統剛換過來，資料庫是從這個網站開始建的 —— 以前用 Google 表單報過名
 * 的人，這裡查不到，還是得填一次完整報名表。不先講，少年會一直去按
 * 「老朋友快速報名」然後查不到資料，以為系統壞了。
 */
export const FIRST_TIME_NOTICE = {
  title: '第一次用這個網站報名，都要填完整報名表',
  body: '我們正在用這個網站重新建檔，之前透過 Google 表單報名過的不算在內。'
    + '填過一次之後，下次報名其他活動就可以用上面的快速報名，不用再填一遍。',
};
